...
from raisimGymTorch.env.bin import allegro_demo as mano
from raisimGymTorch.env.RaisimGymVecEnvOther import RaisimGymVecEnvTest as VecEnv
from raisimGymTorch.helper.raisim_gym_helper import ConfigurationSaver, load_param, tensorboard_launcher
from raisimGymTorch.env.bin.allegro_demo import NormalSampler
...
for update in range(25):
    ...
    if record:
        ...
        pose_r = np.zeros((num_envs, n_steps_r, 22))
    ...
    for step in range(n_steps_r):
        ...
        if record:
            for i in range(num_envs):
                trans_r[i, step, :] = global_state[i, 112:115].reshape(-1,)
                axis, angle = rotations.quat2axisangle(global_state[i, 115:119].reshape(1,4))
                rot_r[i, step, :] = axis * angle
                pose_r[i, step, 6:] = global_state[i, 119:135]
                trans_obj[i, step, :] = global_state[i, 135:138]
                axis, angle = rotations.quat2axisangle(global_state[i, 138:142].reshape(1,4))
    ...
    if record:
        for i in range(num_envs):
            ...
            if not os.path.exists(f"./data_all/allegro_diverse_seq_npy/"):
                os.makedirs(f"./data_all/allegro_diverse_seq_npy/")
            if not os.path.exists(f"./data_all/allegro_diverse_seq_obj/"):
                os.makedirs(f"./data_all/allegro_diverse_seq_obj/")

            np.save(f"./data_all/allegro_diverse_seq_npy/allegro_{obj_item}.npy", data)

            obj_file = directory_path + obj_item + "/top_watertight_tiny.obj"
            shutil.copy(obj_file, f"./data_all/allegro_diverse_seq_obj/allegro_{obj_item}.obj")