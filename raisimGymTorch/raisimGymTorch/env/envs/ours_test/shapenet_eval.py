...
train_cat = 'large_train'
train_path = home_path + f"/rsc/{train_cat}/"
train_items = os.listdir(train_path)
train_folder_names = [item for item in train_items if os.path.isdir(os.path.join(train_path, item))]
for folder in train_folder_names:
...
for update in range(5):
    ...
    for i in range(num_envs):
        ...
        rot = current_label['init_axis_angle']
        pos = current_label['init_pos']
        bias = current_label['target_center']
